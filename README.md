...
- [ ] `Position` 객체를 이곳저곳에서 사용하도록 하자!
- [ ] 사다리 생성 -> 사용자를 입력하면 그때 사다리를 타게 하는게 프로그램의 동작의 흐름 입장에서 더 자연스러울듯
- [ ] 사용자의 이름은 `all`이 될 수 없다. 에약어처럼 처리하자.
- [ ] 더 작은 단위인 `Point`도 객체로 분리할 수 있지 않을까?
