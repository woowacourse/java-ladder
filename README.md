...
<td><ul>
    <li>
        콤마 <code>,</code> 로 구분된 이름이 2개 이상이 아닐 경우 ✔️<br>
    ...
    <li>
        중복된 이름을 입력될 경우 ✔️<br>
        <code>[ERROR] 이름은 중복될 수 없습니다.</code>
    </li>
</ul></td>
</tr>
</table>
...
<td><ul>
    <li>
        질문에 해당하는 아래 메세지 출력️ ✔️<br>
        <code>실행 결과를 입력하세요. (결과는 쉼표(,)로 구분하세요) ✔️</code>
    </li>
    <li>실행 결과를 입력받고 잘 입력 받았는지 체크</li>
</ul></td>
<td><ul>
    <li>
        구분자 <code>,</code> 로 분리 후 개수가 사람들 수와 다를 경우 ✔️️<br>
...
<td><ul>
    ...
    <li>사다리 보상 출력 시 각 보상을 오른쪽 정렬한다. ✔️</li>
    <li>출력 시 사다리 한 세로 줄의 위쪽 이름과 아래쪽 보상의 최대 길이만큼 사다리 발판 길이를 늘리거나 줄인다. ✔️</li>
</ul></td>
</tr>
</table>

### 5️⃣ 결과를 보고 싶은 사람 입력 ✔️
...
<td><ul>
    ...
    <li>사람 이름을 입력받고 잘 입력 받았는지 체크 ✔️</li>
</ul></td>
<td><ul>
    <li>
        보고 싶은 사람이 존재하지 않을 경우 아래 예외 메세지를 출력 후 재입력을 받는다. ✔️<br>
...
<td><ul>
    <li>
        실행 결과 타이틀 메세지 출력 ✔️<br>
        <code>실행 결과</code>
    </li>
    <li>입력받은 값이 <code>exit</code> 일 경우 프로그램 종료 ✔️</li>
    <li>입력받은 사람 이름과 생성한 사다리 및 보상 목록을 통해 실행 결과를 생성 ✔️</li>
    <li>실행 결과 출력 ✔️</li>
...
### 🙋‍♂️ 결과를 보고 싶은 사람 입력 반복 로직을 어떻게 잘 작성하는가?

`all` 이라는 지정된 명령어로 사다리 게임의 모든 결과를 출력하는 로직이 있었습니다.

이를 위해 사람 이름에 `all` 이 들어올 경우 예외가 발생하도록 하였고, `Enum` 을 적절히 사용할 수 있겠다 생각되어 `Command` 를 만들었습니다.

`exit` 라는 예약어도 지정하여 프로그램을 종료할 수 있게 까지 작성을 완료하였는데 이러한 것들을 위해 최종적으로 `CommandController` 에서 수행하도록 하였습니다.

`indent` 를 `1` 로 유지해야하는 요구조건에 따라 새로운 컨트롤러를 만들었는데 이러한 방법이 적절한지 궁금합니다!
