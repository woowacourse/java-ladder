...
- [x] 참여할 사람 이름을 입력받는다.
  ...
  - [x] 이름은 중복될 수 없다.
  - [ ] "all" 이라는 이름은 입력할 수 없다.
- [x] 최대 사다리 높이를 입력받는다.
  - [x] 사다리 높이는 숫자이어야한다.
  - [x] 사다리 높이는 1 이상이어야한다.
- [ ] 실행 결과를 입력받는다.
  - [ ] 쉼표로 구분한다.
  - [ ] 결과의 갯수는 참여한 사람 수와 같아야한다.
  - [ ] 빈값 혹은 공백을 입력할 수 없다.
...
### 사다리 실행
- [ ] 해당 위치 앞뒤에 디딤판이 존재하면 디딤판 방향으로 이동한 뒤 아래칸으로 이동한다.
- [ ] 이동할 수 있는 디딤판이 존재하지 않으면 아래칸으로 이동한다.
- [ ] 사다리 양 끝단에 도달하면 아래칸으로 이동한다.
- [ ] 사다리 맨 아래층에 도달하면 사다리 실행이 종료된다.
