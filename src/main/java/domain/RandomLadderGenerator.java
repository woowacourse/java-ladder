...
public class RandomLadderGenerator implements LadderGenerator {
    ...
    public List<Line> generate(int width, int height) {
        ...
        for (int i = 0; i < height; i++) {
            List<Boolean> movements = generate(width);
     ...
     *
     * <p>
     * 랜덤을 생성하면서, 연속된 true값은 나올 수 없다. 이전 요소값이 true이면 현재 요소값은 무조건 false를 삽입한다. 그렇지 않다면 랜덤값을 삽입한다.
     * </p>
     *
     * @param count 사람수; 0 이상이어야 한다.
     * @return (사람수 - 1) 길이의 true와 false 요소값을 가지는 리스트를 반환한다.
     */
    private List<Boolean> generate(int count) {
