...
// 예외에 관해서 글다시 확인해보고 상수 변수명을 어떻게 써야할 지 고민.
public class InputView implements AutoCloseable {
    ...
    private static final String NOT_ALLOWED_NULL_EMPTY_INPUT = "입력이 null이거나 빈 문자열 일 수 없습니다.";
    private static final String NOT_ALLOWED_NOT_NUMERIC_INPUT = "입력이 숫자가 아니거나 음수로 구성되어 있습니다." + System.lineSeparator() + "0 혹은 양수를 입력해야 합니다. 예: 3";
    ...
    private void validateNotNullAndBlank(String input) {
        if (input == null || input.isBlank()) {
            throw new IllegalArgumentException(NOT_ALLOWED_NULL_EMPTY_INPUT);
        }
    }
    ...
    private void validateNumeric(String input) {
        if (!NUMERIC_PATTERN.matcher(input).matches()) {
            throw new IllegalArgumentException(NOT_ALLOWED_NOT_NUMERIC_INPUT);
        }
    }
