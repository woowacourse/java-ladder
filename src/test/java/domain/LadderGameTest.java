...
class LadderGameTest {
    ...
    @DisplayName("게임의 모든 결과를 정확하게 가져와야 한다.")
    @Test
    void get_all_result() {
        LadderGame ladderGame = createLadderGame();
        Results expected = new Results(List.of(new Result("포비결과"), new Result("글렌결과"), new Result("베로결과")),
                new Names(List.of(new Name("pobi"), new Name("glen"), new Name("vero"))));
        AtomicInteger index = new AtomicInteger();

        Results results = ladderGame.getAllResults();

        results.forEach(result -> assertThat(result).isEqualTo(expected.get(index.getAndIncrement())));
    }
