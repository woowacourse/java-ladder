...
class RewardsTest {
    ...
    void validRewardsTest(String rewards, int personCount) {
        assertDoesNotThrow(() -> new Rewards(rewards, personCount));
    }

    @DisplayName("보상의 입력 개수와 사다리 게임 참가자 수가 다르면 예외가 발생한다.")
    @ParameterizedTest
    @CsvSource(value = {"a,b:3", "a,b,c:2", "a,b,c,d,e:4"}, delimiter = ':')
    void invalidRewardsTest(String rewards, int personCount) {
        assertThatIllegalArgumentException()
                .isThrownBy(() -> new Rewards(rewards, personCount))
