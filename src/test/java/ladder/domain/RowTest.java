...
class RowTest {
    ...
    public void 생성_fail_연속발판() {
        assertThatThrownBy(() -> Row.of(List.of(Step.Y, Step.Y), 2))
    ...
    public void 생성_fail_width사이즈() {
        assertThatThrownBy(() -> Row.of(List.of(Step.N, Step.N, Step.Y), 2))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    public void 발판_생성_가능_여부_판단(Row row, int targetIndex, boolean expected) {
        assertThat(row.isStepPossible(targetIndex)).isEqualTo(expected);
    }

    static Stream<Arguments> 발판_생성_가능_여부_판단_데이터() {
        return Stream.of(
                Arguments.of(Row.of(List.of(Step.Y, Step.N), 2), 1, false),
                Arguments.of(Row.of(List.of(Step.N, Step.N, Step.Y), 3), 0, true),
                Arguments.of(Row.of(List.of(Step.N, Step.N, Step.Y), 3), 1, false)
        );
    }
}
