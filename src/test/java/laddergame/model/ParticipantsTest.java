...
class ParticipantsTest {
    ...
    void Should_Success_When_NameInput() {
        assertDoesNotThrow(() -> new Participants(List.of("name", "name2", "name3")));
    }

    @Test
    @DisplayName("참여자가 한명인 경우 예외 발생")
    void Should_ThrowException_When_OnePerson() {
        assertThatThrownBy(() -> new Participants(List.of("name")))
    ...
    void Should_ThrowException_When_DuplicateNames() {
        assertThatThrownBy(() -> new Participants(List.of("name1", "name1", "name2")))
            .isExactlyInstanceOf(IllegalArgumentException.class)
